...
app = FastAPI(
    ...
    description="Advanced IDS with ML anomaly detection and honeypot capabilities",
    version="1.0.0"
)
...
class IDSEngine:
    ...
    def _train_with_dummy_data(self):
        """Train ML models with dummy data"""
        try:
            # Generate training data
            training_data = []
            for _ in range(100):
                features = [
                    random.randint(64, 1500),  # packet_size
                    random.randint(80, 8080),  # port
                    random.uniform(0.1, 10.0), # duration
                    random.randint(10, 1000),  # payload_length
                    random.randint(1, 5),      # protocol_encoded
                    random.randint(0, 23),     # hour
                    random.randint(0, 1),      # night_flag
                    random.randint(1, 100),    # unique_chars
                    random.randint(0, 10),     # path_separators
                    random.randint(0, 5),      # query_params
                    random.randint(0, 3),      # html_tags
                    random.randint(0, 2),      # script_count
                    random.randint(0, 1)       # sql_keywords
                ]
                label = 1 if random.random() > 0.8 else 0  # 20% malicious
    ...
    def extract_features(self, traffic_data: Dict[str, Any]) -> np.ndarray:
        ...
        features = [
            ...
            traffic_data.get('port', 0),
            traffic_data.get('duration', 0),
            len(payload),
            1,  # protocol encoded
            datetime.now().hour,
            1 if 22 <= datetime.now().hour or datetime.now().hour <= 6 else 0,
            len(set(payload)),
            payload.count('/'),
            payload.count('='),
            payload.count('<'),
    ...
    def anomaly_detection(self, traffic_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"is_anomaly": False, "confidence": 0.0, "method": "ML_Error", "error": str(e)}
...
def generate_dummy_alerts():
    ...
    for i in range(50):
        ...
        alert = {
            ...
            "description": f"Security threat detected via ML analysis and pattern matching",
...
async def startup_event():
    ...
    init_data()
    print("✅ IDS System with ML capabilities initialized successfully")
...
async def root():
    return {
        ...
        "version": "1.0.0",
        "features": {
            "ml_anomaly_detection": ids_engine.is_trained,
...
@app.get("/api/alerts")
@limiter.limit("30/minute")
async def get_alerts(request: Request, severity: str = None, limit: int = 50):
    """Get IDS alerts"""
    try:
...
async def add_regex_rule(request: Request, rule_data: dict):
...
async def get_system_status(request: Request):
    ...
    return {
        ...
        "ml_status": {
            "models_trained": ids_engine.is_trained,
            "isolation_forest": "Active" if ids_engine.is_trained else "Inactive",
            "random_forest": "Active" if ids_engine.is_trained else "Inactive"
        }
    }
...
async def simulate_traffic(request: Request, traffic_type: str = "mixed", count: int = 10):
    """Generate and analyze new traffic with ML"""
    if count > 50:
    ...
    return {
        "status": "success",
        "message": f"Generated and analyzed {len(new_traffic)} traffic records with ML",
        "ml_enabled": ids_engine.is_trained,
...
async def analyze_traffic(request: Request, traffic_data: dict):
    """Analyze specific traffic with ML and pattern detection"""
    try:
